  ...
  try {
    ...
    res.status(result.reused ? 200 : 201).json({
      message: result.reused
        ? "Thank you. Your enquiry is already with our team, and we have linked this conversation to the existing record."
...
(function(){
  ...
  var state = { open:false, tab:'home', formMode:'support', prompt:true, config:null, messages:[], loading:false, conversationId:null, enquirySuccess:null };
  function esc(v){return String(v||'').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  function icon(name){var icons={home:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 10.5 12 4l8 6.5V20a1 1 0 0 1-1 1h-5v-6h-4v6H5a1 1 0 0 1-1-1z"/></svg>',chat:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 6.5h14a2 2 0 0 1 2 2v6.5a2 2 0 0 1-2 2H11l-5 3v-3H5a2 2 0 0 1-2-2V8.5a2 2 0 0 1 2-2z"/><circle cx="8" cy="11.5" r="1.1"/><circle cx="12" cy="11.5" r="1.1"/><circle cx="16" cy="11.5" r="1.1"/></svg>',contact:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 4h3l1.5 4-2 1.2a10 10 0 0 0 5.3 5.3l1.2-2 4 1.5v3a2 2 0 0 1-2.2 2A15.5 15.5 0 0 1 5 6.2 2 2 0 0 1 7 4z"/></svg>',support:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 12a6 6 0 0 1 12 0v4a3 3 0 0 1-3 3h-2"/><path d="M4 12h3v5H4zM17 12h3v5h-3z"/><circle cx="12" cy="19" r="1.4"/></svg>',send:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 5l18 7-18 7 3-7z"/></svg>',bubble:'<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M14 17h36a7 7 0 0 1 7 7v15a7 7 0 0 1-7 7H31l-12 8v-8h-5a7 7 0 0 1-7-7V24a7 7 0 0 1 7-7z"/><circle cx="24" cy="32" r="3"/><circle cx="32" cy="32" r="3"/><circle cx="40" cy="32" r="3"/></svg>'};return icons[name]||'';}
  function styles(c){var side=c.widgetPosition==='left'?'left':'right';var accent='#0ea5e9';return '<style>:host{all:initial;font-family:Inter,Arial,sans-serif;color:#0f172a}.cw-btn{position:fixed;z-index:2147483646;bottom:22px;'+side+':22px;width:76px;height:76px;border-radius:999px;border:5px solid #fff;background:#f8fcff;color:'+accent+';box-shadow:0 18px 45px rgba(15,23,42,.25);cursor:pointer;display:grid;place-items:center}.cw-btn svg{width:40px;height:40px;fill:#a9d9f2;stroke:#0b79bd;stroke-width:2}.cw-nudge{position:fixed;z-index:2147483645;bottom:92px;'+side+':42px;min-width:280px;max-width:340px;background:#fff;border:1px solid #7aa7df;border-radius:14px;padding:16px 44px 16px 20px;box-shadow:0 16px 40px rgba(15,23,42,.18);font:600 16px/1.35 Arial;color:#111827}.cw-nudge:after{content:"";position:absolute;bottom:-11px;'+side+':24px;width:20px;height:20px;background:#fff;border-right:1px solid #7aa7df;border-bottom:1px solid #7aa7df;transform:rotate(45deg)}.cw-nudge-close{position:absolute;right:14px;top:12px;border:0;background:transparent;color:#64748b;font-size:24px;cursor:pointer}.cw-panel{position:fixed;z-index:2147483646;bottom:108px;'+side+':22px;width:min(560px,calc(100vw - 20px));height:min(640px,calc(100vh - 132px));background:#fff;border:1px solid '+accent+';border-radius:10px;box-shadow:0 22px 70px rgba(15,23,42,.28);overflow:hidden;display:flex;flex-direction:column}.cw-head{padding:24px 36px;background:'+accent+';color:#fff}.cw-title{font-size:26px;font-weight:900}.cw-sub{display:none}.cw-close{float:right;border:0;background:transparent;font-size:22px;cursor:pointer;color:#fff}.cw-body{flex:1;overflow:auto;padding:16px;background:#fff}.cw-footer{padding:14px 16px;background:#fff;box-shadow:0 -10px 24px rgba(15,23,42,.08)}.cw-send-row{display:flex;border:1px solid #60a5fa;border-radius:4px;overflow:hidden;background:#fff}.cw-send-row .cw-input,.cw-send-row .cw-text{border:0;border-radius:0;margin:0;min-height:54px}.cw-send-btn{width:68px;border:0;background:#1f2937;color:#fff;cursor:pointer;display:grid;place-items:center}.cw-send-btn svg{width:34px;height:34px;fill:#fff}.cw-nav{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #dbeafe;border-radius:14px 14px 0 0;background:#f8fafc;box-shadow:0 -8px 24px rgba(15,23,42,.08)}.cw-nav button{border:0;background:transparent;padding:14px 4px 12px;font-size:14px;font-weight:700;color:#334155;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:5px}.cw-nav button svg{width:26px;height:26px;fill:#dff3ff;stroke:#0b79bd;stroke-width:1.8}.cw-nav button.active{color:#0f172a;background:#eef6ff}.cw-card,.cw-input,.cw-text{width:100%;box-sizing:border-box;border:1px solid #dbeafe;border-radius:6px;background:#fff;padding:12px;margin:7px 0;font:600 15px Arial;color:#334155}.cw-card{cursor:pointer;text-align:left}.cw-primary{width:100%;border:0;border-radius:8px;padding:12px;background:'+accent+';color:#fff;font-weight:900;cursor:pointer;margin-top:8px}.cw-msg{margin:8px 0;max-width:86%;padding:10px 12px;border-radius:14px;font:600 13px/1.4 Arial}.cw-user{margin-left:auto;background:'+accent+';color:#fff}.cw-ai{background:#f8fafc;border:1px solid #dbeafe;color:#334155}.cw-small{font-size:12px;color:#64748b;font-weight:700}.cw-success{margin-top:10px;border:1px solid #bbf7d0;background:#f0fdf4;color:#166534;border-radius:8px;padding:12px;font-size:13px;font-weight:800;line-height:1.45}.cw-error{margin-top:10px;border:1px solid #fecaca;background:#fef2f2;color:#b91c1c;border-radius:8px;padding:12px;font-weight:800;font-size:12px;line-height:1.45}@media(max-width:520px){.cw-panel{right:10px;left:10px;width:auto;height:min(620px,calc(100vh - 120px))}.cw-nudge{right:26px;left:26px;min-width:0}.cw-title{font-size:22px}}</style>'}
  function render(){
    var c = state.config && state.config.configuration;
    if(!c){ shadow.innerHTML='<style>.cw-btn{position:fixed;right:22px;bottom:22px;width:76px;height:76px;border-radius:999px;border:5px solid #fff;background:#f8fcff;color:#0ea5e9}</style><button class="cw-btn" aria-label="Loading chat">...</button>'; return; }
    var html = styles(c) + '<button class="cw-btn" id="cw-toggle" aria-label="Open chat">'+icon('bubble')+'</button>';
    if(!state.open && state.prompt){ html += '<div class="cw-nudge">Need help or want to talk to us?<button class="cw-nudge-close" id="cw-nudge-close" aria-label="Close help prompt">x</button></div>'; }
    if(state.open){
      html += '<section class="cw-panel" role="dialog" aria-label="'+esc(c.chatbotName)+'"><header class="cw-head"><button class="cw-close" id="cw-close" aria-label="Close">x</button><div class="cw-title">Chat With Us</div><div class="cw-sub">'+esc(c.greeting)+' '+esc(c.subtitle)+'</div></header><main class="cw-body">'+tabHtml()+'</main><nav class="cw-nav">'+['home','chat','contact','support'].map(function(t){return '<button data-tab="'+t+'" class="'+(state.tab===t?'active':'')+'">'+icon(t)+'<span>'+t.charAt(0).toUpperCase()+t.slice(1)+'</span></button>'}).join('')+'</nav></section>';
    }
    shadow.innerHTML = html; bind();
  }
  function tabHtml(){
    var cfg = state.config.configuration;
    if(state.tab==='home') return '<div class="cw-card" style="font-size:20px;font-weight:500;padding:16px 20px;margin-bottom:22px">How can I help you today?</div>'+((cfg.actionCards||[]).filter(function(x){return x.enabled!==false}).map(function(card){return '<button class="cw-card" data-action="'+esc(card.id||'chat')+'">'+esc(card.label)+'</button>'}).join('') || '<p class="cw-small">Start a chat or contact the team.</p>');
    if(state.tab==='chat') return '<div id="cw-messages">'+state.messages.map(function(m){return '<div class="cw-msg '+(m.sender==='user'?'cw-user':'cw-ai')+'">'+esc(m.text)+'</div>'}).join('')+'</div>'+(state.config.suggestedQuestions||[]).slice(0,3).map(function(q){return '<button class="cw-card cw-suggest">'+esc(q)+'</button>'}).join('')+'<div class="cw-footer"><div class="cw-send-row"><textarea class="cw-text" id="cw-chat-input" rows="1" placeholder="Your Query"></textarea><button class="cw-send-btn" id="cw-send" aria-label="Send message">'+icon('send')+'</button></div></div>';
    ...
    if(state.formMode==='enquiry' && state.enquirySuccess) return '<div class="cw-success" role="status"><b>Enquiry submitted successfully</b><br>'+esc(state.enquirySuccess.message||'Thank you. Your enquiry has been submitted successfully. Our team will contact you shortly.')+'</div><button class="cw-primary" data-tab="chat">Continue Chat</button><button class="cw-card" id="sup-new-enquiry">Submit another enquiry</button>';
  ...
  function bind(){
    var toggle=shadow.getElementById('cw-toggle'); if(toggle) toggle.onclick=function(){state.open=!state.open; state.prompt=false; render();};
    var close=shadow.getElementById('cw-close'); if(close) close.onclick=function(){state.open=false; render();};
    var nudgeClose=shadow.getElementById('cw-nudge-close'); if(nudgeClose) nudgeClose.onclick=function(){state.prompt=false; render();};
    ...
    var newEnq=shadow.getElementById('sup-new-enquiry'); if(newEnq) newEnq.onclick=function(){state.enquirySuccess=null;state.formMode='enquiry';render();};
  }
  ...
  function submitEnquiry(){
    ...
    fetch(apiBase+'/'+encodeURIComponent(widgetId)+'/enquiry',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json().then(function(j){if(!r.ok) throw j; return j;});}).then(function(j){state.conversationId=j.conversationId; state.enquirySuccess={message:j.message,enquiryId:j.enquiryId}; render();}).catch(function(e){var el=shadow.getElementById('sup-result'); if(el){el.innerHTML='<div class="cw-error">'+esc((e.errors&&e.errors[0]&&e.errors[0].msg)||'Could not submit enquiry. Please check the required fields and try again.')+'</div>';}});
  }
