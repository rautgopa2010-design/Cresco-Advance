...
(function(){
  ...
  var state = { open:false, tab:'home', formMode:'support', prompt:true, config:null, messages:[], loading:false, conversationId:null };
  ...
  function submitEnquiry(){
    ...
    fetch(apiBase+'/'+encodeURIComponent(widgetId)+'/enquiry',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json().then(function(j){if(!r.ok) throw j; return j;});}).then(function(j){state.conversationId=j.conversationId; var el=shadow.getElementById('sup-result'); if(el){el.innerHTML='<div class="cw-success">'+esc(j.message||'Thank you. Your enquiry has been submitted successfully. Our team will contact you shortly.')+'</div>';}}).catch(function(e){var el=shadow.getElementById('sup-result'); if(el){el.innerHTML='<div class="cw-error">'+esc((e.errors&&e.errors[0]&&e.errors[0].msg)||'Could not submit enquiry. Please check the required fields and try again.')+'</div>';}});
  }
