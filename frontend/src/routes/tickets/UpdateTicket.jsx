...
import { getTickets, updateTicket } from "../../redux/actions/ticket";
...
const UpdateTicket = () => {
    ...
    const { snackbarMessage, snackbarSeverity, ticket: tickets = [], loading } = useSelector((state) => state.ticket);
    ...
    useEffect(() => {
        if (tickets && tickets.length > 0 && id) {
            const currentTicket = tickets.find((t) => String(t.id) === String(id));
            if (currentTicket) {
                ...
                const matchedEmployees = employees.filter((emp) => {
                    ...
                    return currentTicket.assignedTo?.includes(fullName);
                });

                const parseDateForInput = (dateStr) => {
                    if (!dateStr) return today.forInput;
                    const [dd, mm, yyyy] = dateStr.split("-");
                    return `${yyyy}-${mm}-${dd}`;
                };

                const linkedOrder = orders.find((o) => o.id === currentTicket.orderId) || null;

                setForm({
                    date: parseDateForInput(currentTicket.createdDate),
                    ticketDeadline: parseDateForInput(currentTicket.dueDate),
    ...
    }, [tickets, employees, orders, id, today.forInput]);
    ...
    const handleSubmit = () => {
        ...
        dispatch(updateTicket(id, submissionData));
            ...
            {initialLoad ? (
                ...
                <div className="card space-y-4">
                    <div className="flex items-center justify-between text-nowrap">
                        <div className="text-base font-semibold text-[#433C50] md:text-lg">Update Ticket :</div>
                        <Button
                            onClick={() => navigate(-1)}
                            variant="gradient"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-end">
                            {/* <Button
                                ...
                                variant="gradient"
                                className="flex items-center gap-2 rounded bg-green-600 px-2 py-2 text-xs capitalize md:px-3 md:text-base lg:px-3 lg:text-base"
                            >
                                <IoTicket size={20} />
                                Update Ticket
                            </Button> */}
