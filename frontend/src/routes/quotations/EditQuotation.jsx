...
import { updateQuotation, getQuotations } from "../../redux/actions/quotation";
...
const EditQuotation = () => {
    ...
    const { quotations, snackbarMessage, snackbarSeverity, loading } = useSelector((state) => state.quotation);
...
useEffect(() => {
    // Only set defaults if we're not editing an existing quotation
    if (!id && defaultEntries?.quotation_description?.content && !form.quotationDescription) {
        setForm(prev => ({
            ...prev,
            quotationDescription: defaultEntries.quotation_description.content
        }));
    }
    if (!id && defaultEntries?.quotation_terms?.content && !form.termsAndConditions) {
        setForm(prev => ({
            ...prev,
    ...
    useEffect(() => {
        ...
        const quotationData = quotations.find((q) => q.id === Number(id));
        if (!quotationData) return;

        // === 1. Assigned To ===
        const matchedEmployees = employees.filter((emp) => {
            const fullName = `${emp.salutation} ${emp.firstName} ${emp.middleName || ""} ${emp.lastName}`.trim();
            return quotationData.assignedTo?.includes(fullName);
        });
        ...
        setForm((prev) => ({
            ...
            selectedCustomer: quotationData.customerPerson || "",
            date: quotationData.date ? quotationData.date.split("-").reverse().join("-") : "",
            gstinType: quotationData.gstinType || "Intrastate",
            quotationDescription: quotationData.description || "",
            termsAndConditions: quotationData.termsAndConditions || "",
            finalAmt: quotationData.finalAmt || "",
        }));

        // === 3. Company & Address Details ===
        const billingCountryId = country.find((c) => c.country === quotationData.billingAddress?.country)?.id || "";
        const shippingCountryId = country.find((c) => c.country === quotationData.shippingAddress?.country)?.id || "";
        ...
        setCompanyDetails({
            ...
            billingAddress: {
                ...
                zone: quotationData.billingAddress?.zone || "",
            },
            shippingAddress: {
                ...
                zone: quotationData.shippingAddress?.zone || "",
            },
        });

        setSelectedCompany(quotationData.companyName || "");

        // === 4. Mobile & Email ===
        const [code = "", ...mobileParts] = (quotationData.mobile || "").split(" ");
        ...
        // === 5. Product Details (Intrastate & Interstate) ===
        const productDetails = quotationData.productQuotationDetails || {
            intrastate: [],
            interstate: [],
        };
        ...
        if (businessMode === "B2C" && quotationData.customerPerson) {
            const targetName = quotationData.customerPerson.trim();
        ...
        // === 6. PREFILL COMPANY & CUSTOMER AUTOCOMPLETE (Critical Fix) ===
        const targetCompanyName = quotationData.companyName?.trim();
        const targetCustomerName = quotationData.customerPerson?.trim();

        let matchingCompanyOption = null;

        if (targetCompanyName && targetCustomerName) {
            // Priority: Match company name + customer person exists in contactList
        ...
        // Fallback 1: Match only by company name
        ...
        // Fallback 2: Create temporary option if no match found (rare case)
        if (!matchingCompanyOption && targetCompanyName) {
        ...
        if (matchingCompanyOption) {
            ...
            // Safety: Ensure customer name is in dropdown even if slight mismatch
        ...
        // === 7. AUTO DETECT BUSINESS MODE (B2B vs B2C) ===
        const savedCompanyName = quotationData.companyName?.trim();
        const savedCustomerName = quotationData.customerPerson?.trim();

        if (savedCompanyName && savedCustomerName) {
            // Try to find if this company + customer combination exists in customers data
            const matchingCompany = customers.find((cust) => {
            ...
            // If there's a customer but no/no-matching company → likely B2C
            setBusinessMode("B2C");
        } else {
            // Fallback
            setBusinessMode("B2B");
        }
    }, [id, quotations, customers, companyOptions, employees, country, zones]);
    ...
    const handleSubmit = () => {
        ...
        const result = {
            ...
            // ✅ Use the existing selectedCustomerDetails values instead of companyDetails.customers
            mobile: `${selectedCustomerDetails.code} ${selectedCustomerDetails.mobile}`.trim(),
        ...
        dispatch(updateQuotation(id, result));
