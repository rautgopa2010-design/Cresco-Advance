...
import { getProductUnit, createProductUnit, updateProductUnit, deleteProductUnit } from "../../redux/actions/productUnit";
...
const ProductUnit = () => {
    ...
    const loading = useSelector((state) => state.productUnit.loading);

    const { snackbarMessage, snackbarSeverity } = useSelector((state) => state.productUnit);
    const [snackbarOpen, setSnackbarOpen] = useState(false);

    const productUnitList = useSelector((state) => state.productUnit.productUnit);
    ...
    const handleEdit = (productUnit) => {
        setProductUnit(productUnit.productUnit);
        setEditId(productUnit.id);
        setIsEditMode(true);
        setOpen(true);
        setError(false);
    };
    ...
    const handleUpdate = async () => {
        if (!productUnit.trim()) {
            setError(true);
            setLocalSnackbarMessage("Product Unit is required");
            setLocalSnackbarSeverity("error");
            setSnackbarOpen(true);
            return;
        }
        dispatch(
            updateProductUnit(editId, {
    ...
    const confirmDelete = async () => {
        dispatch(deleteProductUnit(selectedDeleteId));
            ...
            {loading ? (
                ...
                <div className="card">
                    <div className="flex items-center justify-between text-nowrap">
                        <div className="text-base font-semibold text-[#433C50] md:text-lg lg:text-lg">Product Unit :</div>
                        <Button
                            variant="gradient"
                            ...
                            <table className="table">
                                <thead className="table-header text-nowrap bg-[#053054] text-white">
                                    <tr className="table-row">
                                        ...
                                        <th className="table-head border border-gray-300 capitalize">Product Unit</th>
                                ...
                                <tbody className="table-body text-[#433C50]">
                                    {productUnitList.map((productUnit, index) => (
                                        <tr
                                            className="table-row"
                                            key={index}
                                        >
                                            <td className="table-cell border border-gray-300">{index + 1}</td>
                                            <td className="table-cell border border-gray-300">{productUnit.productUnit}</td>
                                            <td className="table-cell border border-gray-300">{productUnit.date}</td>
                                            <td className="table-cell border border-gray-300">
                                                <div className="flex items-center gap-x-4">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => handleDeleteClick(productUnit.id)}
                                                    >
                                                        <Trash size={20} />
                                                    </button>
                                                </div>
            ...
            >
                <Box sx={modalStyle}>
                    <div className="mb-4 flex items-center justify-between">
                        ...
                        >
                            {isEditMode ? "Update Product Unit" : "Add Product Unit"}
                        </Typography>
