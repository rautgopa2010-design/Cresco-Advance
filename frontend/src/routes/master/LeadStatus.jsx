...
//     useEffect(() => {
//         const storedLeads = JSON.parse(localStorage.getItem("leadStatusList")) || [];
...
//         if (!leadStatus.trim()) {
//             setError(true);
//             setSnackbarMessage("Lead Status is required");
...
//             ...updatedLeads[editIndex],
//             name: leadStatus.trim(),
//             date: getCurrentISTDateTime(), // optionally update timestamp
//         };

//         setLeadList(updatedLeads);
//         localStorage.setItem("leadStatusList", JSON.stringify(updatedLeads));
...
//         setSnackbarMessage("Lead Status Created Successfully");
//         setSnackbarOpen(true);
//         handleClose();
//     };
...
//                     <div className="text-base font-semibold text-[#433C50] md:text-lg lg:text-lg">Lead Status :</div>
//                     <Button
//                         variant="gradient"
...
//                                     <th className="table-head border border-gray-300 capitalize">Lead Status No.</th>
//                                     <th className="table-head border border-gray-300 capitalize">Lead Status Id</th>
//                                     <th className="table-head border border-gray-300 capitalize">Lead Status</th>
...
//                         >
//                             {isEditMode ? "Update Lead Status" : "Add Lead Status"}
//                         </Typography>
...
import { getLeadStatus, createLeadStatus, updateLeadStatus, deleteLeadStatus } from "../../redux/actions/leadStatus";
...
const LeadStatus = () => {
    ...
    const loading = useSelector((state) => state.leadStatus.loading);

    const { snackbarMessage, snackbarSeverity } = useSelector((state) => state.leadStatus);
    const [snackbarOpen, setSnackbarOpen] = useState(false);

    const leadStatusList = useSelector((state) => state.leadStatus.leadStatus);
    ...
    const handleEdit = (leadStatus) => {
        setLeadStatus(leadStatus.leadStatus);
        setEditId(leadStatus.id);
        setIsEditMode(true);
        setOpen(true);
        setError(false);
    };
    ...
    const handleUpdate = async () => {
        if (!leadStatus.trim()) {
            setError(true);
            setLocalSnackbarMessage("Lead Status is required");
            setLocalSnackbarSeverity("error");
            setSnackbarOpen(true);
            return;
        }
        dispatch(
            updateLeadStatus(editId, {
    ...
    const confirmDelete = async () => {
        dispatch(deleteLeadStatus(selectedDeleteId));
        ...
        <>
                    ...
                    <div className="card-body p-0">
                        <div className="relative w-full flex-shrink-0 overflow-auto rounded-none [scrollbar-width:_thin]">
                            <table className="table">
                                ...
                                <tbody className="table-body text-[#433C50]">
                                    {leadStatusList.map((leadStatus, index) => (
                                        <tr
                                            className="table-row"
                                            key={index}
                                        >
                                            <td className="table-cell border border-gray-300">{index + 1}</td>
                                            <td className="table-cell border border-gray-300">{leadStatus.leadStatus}</td>
                                            <td className="table-cell border border-gray-300">{leadStatus.date}</td>
                                            <td className="table-cell border border-gray-300">
                                                <div className="flex items-center gap-x-4">
                                                    <button
                                                        className="text-blue-500"
                                                    ...
                                                    <button
                                                        className="text-red-500"
                                                        onClick={() => handleDeleteClick(leadStatus.id)}
                                                    >
                                                        <Trash size={20} />
                                                    </button>
                                                </div>
            ...
            >
                <Box sx={modalStyle}>
                    ...
                    <Typography className="mb-6 justify-self-center text-[#433C50]">Are you sure, You want to delete this lead status?</Typography>
